    ...
    public abstract class ObservableObject : INotifyPropertyChanged
    {
        ...
        protected virtual void RaisePropertyChanged(PropertyChangedEventArgs eventArgs)
        {
            ...
            catch (Exception e)
            {
                    ...
                    TEngine.Log.Warning("Set property '{0}', raise PropertyChanged failure.Exception:{1}", eventArgs.PropertyName, e);
            }
        }
        ...
        protected virtual void RaisePropertyChanged(params PropertyChangedEventArgs[] eventArgs)
        {
            foreach (var args in eventArgs)
            {
                ...
                catch (Exception e)
                {
                        ...
                        TEngine.Log.Warning("Set property '{0}', raise PropertyChanged failure.Exception:{1}", args.PropertyName, e);
                }
            }
        }
        ...
        {
            if (type.IsValueType)
                TEngine.Log.Debug("Please use Set(field,newValue) instead of Set<T>(field,newValue) to avoid value types being boxed.");
        }
